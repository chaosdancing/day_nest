...
- 一年后翻回去，按时光轴看到"那段时间发生过什么"
- 想看"所有带 樱花 标签的回忆"，进入软木板视图一目了然
- 全家成员都能上传、编辑、浏览

## 2. 范围与非目标

**包含**：账号登录、集合 CRUD、照片上传与浏览、标签系统、时光轴 / 软木板 / 集合详情三种浏览视图、移动端体验。
...
Collection
  ...
  occurred_on           // 集合的"日期"（用户填写的回忆时间），用于时光轴排序
...
| `/` | 时光轴主页 | 默认入口，按 `occurred_on` 倒序，左右交错拍立得布局 |
...
- 主文字：`#2b2418`
- 强调色：`#a88a5c`（牛皮绳色），用于强调标签、时光轴轴线、按钮
...
技术：Framer Motion 为主，必要时配合 GSAP（如时光轴的 scroll trigger）。**移动端禁用过重动效**，仅保留入场淡入、点击反馈。

**关键动画清单**

| 场景 | 动画 |
| --- | --- |
| 时光轴卡片入场 | 滚动到视口时，`y: 20 → 0` + `opacity 0 → 1` + 轻微旋转回正（150ms 错位） |
| 拍立得 hover | `scale: 1 → 1.04` + 旋转回 0°（"被拿起来看"），投影变大 |
| 集合卡片点击 | shared-layout 动画（Framer Motion `layoutId`），封面从时光轴位置"飞"到集合详情顶图 |
...
- 主页能看到所有集合，按 `occurred_on` 倒序，时光轴布局正确
