...
朝夕居家庭照片记忆，目前只能在 web 看。家人日常用微信，**进出 web 站要跳浏览器 + 输密码 + 不能从相册直接传**，这层摩擦让 daynest 在「随手记一笔家庭日常」场景上被冷落。

小程序的价值是把朝夕居塞进微信对话流里——长辈在群里聊到「上次去苏州那次」可以一秒打开看，妈妈做完饭手机相册多了几张就能直接传。

### 0.2 v1 用户故事

| Persona | 故事 |
|---|---|
| **奶奶**（看图人格） | 从家族群里点开「朝夕居」名片 → 进入时间轴 → 滑到「小宝周岁」集合 → 拍立得轮播 + 双指放大照片 |
| **妈妈**（生产者人格） | 相册多了 8 张家宴照片 → 长按桌面小程序 → 「上传」 → 选 8 张 → 起标题 → 自动按 EXIF 排时间 → 打几个标签 → 完成 |
| **爸爸**（管理者人格） | 邀请姑姑加入家庭 → 邀请链接生成 → 转发到「朝夕居」名片 → 姑姑微信快登 → 输入接收方的 daynest 账密完成绑定 |

### 0.3 目标（Goals）

- 与 web 端**功能等价**：登录/注册/邀请 · 时间轴 · 集合详情 · 拍立得照片浏览（双指缩放） · 上传 · 标签管理 · 收藏 · 设置 · 个人信息编辑
...
BindPage 提供"我还没有朝夕居账号"链接 → `[RegisterPage]`：
...
- 时间轴 + wifi → 静默预拉 collection + upload 包
...
| 时间轴竖虚线 | WXSS dashed border + custom pattern | 0 KB |
...
| R6：小程序分享给陌生人，预览图暴露 | 低 | 中 | 分享卡片只显示品牌字「朝夕居 · 收纳烟火日常」，不带内容预览 |
...
![时间轴](./assets/miniapp/mockup-timeline.png)

*图 1 · 时间轴。拍立得叠加 + 时间轴竖线 + 胶带 pill 滤镜 + 自定义 tabBar。*
